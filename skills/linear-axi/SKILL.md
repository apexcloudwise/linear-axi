---
name: linear-axi
description: "Operate Linear (linear.app) through the linear-axi CLI - list and view issues, create, update, and delete issues, nest work as sub-issues, post, edit, and delete comments and read comment threads, list teams, list/filter projects, list cycles, and list labels. Use whenever a task touches Linear: triaging or filing issues, changing issue state, setting priority, assigning issues or managing their labels, breaking an issue into sub-issues with `issue create --parent` and reading them back with `issue view --full`, commenting on or reading an issue discussion via `comment list`, editing or removing a comment via `comment update/delete <id>` (ids come from `comment list`), discovering team keys via `teams`, scoping work to a project via `projects` and `issues --project`, to a cycle via `cycles` and `issues --cycle`, assigning issues to a project or cycle with `issue create/update --project <name>` and `--cycle current|<number>`, or finding label names via `labels` for `issue create --label`, `issue update --label`, and `issues --label`."
...
Use linear-axi whenever a task touches Linear: listing, viewing, filing, editing, or deleting issues; changing issue state, priority, assignee, or labels; nesting issues as sub-issues with `issue create --parent <ref>`; commenting on issues, editing or deleting comments by id, or reading their comment threads; discovering team keys; listing projects and scoping issues to one with `issues --project <name>`; listing a team's cycles and scoping issues to one with `issues --cycle current|<number> --team <KEY>`; putting issues in a project or cycle with `issue create/update --project <name>` or `--cycle current|<number>`; or listing the workspace's labels and using their names with `issues --label <name>`, `issue create --label <name>`, or `issue update --label <name>`.
...
4. Create an issue with `issue create --title "..." --team <KEY>`; team keys come from `teams`, `--project "<name>"` / `--cycle current|<number>` land it in a project or cycle straight away, and `--parent LIN-123` nests it as a sub-issue.
...
6. Comment with `comment <IDENTIFIER> --body "..."` or `--body-file <path>` for multi-line bodies; read an issue's thread with `comment list <IDENTIFIER>` (newest first, `--full` for untruncated bodies). Each row carries the comment's id - edit with `comment update <COMMENT-ID> --body "..."`, remove with `comment delete <COMMENT-ID>` (deleting a missing comment is a reported no-op).
...
- Mutations are idempotent: updating an issue to its current state, or deleting an already-deleted issue or comment, is a reported no-op (exit 0). Updating a missing comment instead fails loud - only delete is idempotent.
...
- `issue view` truncates the description by default; pass `--full` to see it entirely and list the issue's sub-issues (one `identifier | title | state` line each). `comment list` likewise previews each comment body; `--full` shows them whole. Every `comment list` row starts with the comment's full id - the handle `comment update`/`comment delete` take.
- `issue create --parent <IDENTIFIER|UUID>` creates a sub-issue under the referenced parent (fails loud when the parent is not found; the parent may belong to another team - Linear's hierarchy spans teams).
